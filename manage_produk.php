...
<body class="bg-dark">
    ...
    <script>
        ...
        $(document).ready(function () {
            ...
            $('#dataTable').on('click', '.delete-game-btn', function() {
                var gameId = $(this).closest('tr').find('td:first').text(); // Get the game ID from the table
                if (confirm('Are you sure you want to delete game with ID ' + gameId + '?')) {
                    // Send a deletion request to the server using AJAX
                    $.ajax({
                        type: 'post',
                        url: 'delete_game.php', // Replace with the correct URL for your deletion endpoint
                        data: { gameId: gameId },
                        success: function (response) {
                            // Show success message or perform other actions upon successful deletion
                            alert('Game with ID ' + gameId + ' has been deleted.');
                            // Refresh the page after deletion
                            location.reload();
                        },
                        error: function (xhr, status, error) {
                            // Handle errors or perform other actions if deletion fails
            ...
            $('#dataTable').on('click', '.edit-admin-btn', function() {
