...
if (isset($_SESSION['user'])) {
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['statusOpt']) && $_POST['statusOpt'] !== "") {
        ...
        // if ($selectedValue == -1) {
        //     $stmt = $pdo->query(
        //         "SELECT users.username, game.game_name, item.harga_satuan, invoice.tanggal_transaksi, invoice.invoice_id FROM users
        //         INNER JOIN invoice
        //             ON users.member_id = invoice.member_id
        //         INNER JOIN item
        //             ON invoice.item_id = item.item_id
        //         INNER JOIN game
        //             ON item.game_id = game.game_id WHERE invoice.status_transaksi= -1;"
        //     );
        ...
        if ($selectedValue === -1) {
            $stmt = $pdo->query(
                "SELECT users.username, game.game_name, item.harga_satuan, invoice.status_transaksi, invoice.tanggal_transaksi, invoice.invoice_id FROM users
                INNER JOIN invoice
                    ON users.member_id = invoice.member_id
                INNER JOIN item
                    ON invoice.item_id = item.item_id
        ...
        $stmt = $pdo->query(
            "SELECT users.username, game.game_name, item.harga_satuan, invoice.tanggal_transaksi, invoice.status_transaksi, invoice.invoice_id FROM users
            INNER JOIN invoice
                ON users.member_id = invoice.member_id
            INNER JOIN item
                ON invoice.item_id = item.item_id
            INNER JOIN game
                ON item.game_id = game.game_id;"
        );
        $rows = $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
} else {
